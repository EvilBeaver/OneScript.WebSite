...
Процедура ОбработатьАрхив(Контекст, Сегменты)
	Если Сегменты.Количество() = 2 Тогда // api/archive
		СписокДоступныхВерсий(Контекст);
...
Процедура СписокДоступныхВерсий(Контекст)
	ФлагВПараметре = Контекст.Запрос.Параметры.Получить("all");
	ФильроватьСпециальные = ФлагВПараметре = Неопределено или ФлагВПараметре <> "true";
	ТаблицаВерсий = АрхивВерсий.ТокеныВерсий(ФильроватьСпециальные);

	ЗаписьJSON = Новый ЗаписьJSON();
	ЗаписьJSON.УстановитьСтроку(Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет));
	ЗаписьJSON.ЗаписатьНачалоМассива();
	Для Каждого ОписаниеВерсии Из ТаблицаВерсий Цикл
		ЗаписьJSON.ЗаписатьНачалоОбъекта();

		ЗаписьJSON.ЗаписатьИмяСвойства("token");
		ЗаписьJSON.ЗаписатьЗначение(ОписаниеВерсии.Токен);
		
		ЗаписьJSON.ЗаписатьИмяСвойства("presentation");
		ЗаписьJSON.ЗаписатьЗначение(ОписаниеВерсии.Представление);
		
		ЗаписьJSON.ЗаписатьИмяСвойства("link");
		ЗаписьJSON.ЗаписатьЗначение("/downloads/archive/" + ОписаниеВерсии.Токен);
